...
Содержание  
[Общая архитектура](#1-общая-архитектура)  
[Калибровка датчиков](#2-калибровка-датчиков)  
[Фильтрация сигналов](#3-фильтрация-сигналов)  
[Спектральные преобразования](#4-спектральные-преобразования)  
[Параметрический анализ](#5-параметрический-анализ)  
[Подготовка данных для модального анализа](#6-подготовка-данных-для-модального-анализа)  
[Интеграция и пример использования](#7-интеграция-и-пример-использования)
...
Алгоритм:  
Решить систему $`\mathbf{X} \mathbf{a} = -\mathbf{x}`$ методом QR.  
Найти корни полинома $`P(z) = z^p + a_0 z^{p-1} + \dots + a_{p-1}`$.  
Извлечь параметры:  
...
## 7. Интеграция и пример использования
